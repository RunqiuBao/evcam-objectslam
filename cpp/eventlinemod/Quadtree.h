...
public:
    Quadtree(const Box<Float>& box, const GetBox& getBox = GetBox(),
        ...
        mBox(box), mRoot(std::make_shared<Node>()), mGetBox(getBox), mEqual(equal)
    {
        numNodes++;
    }
    ...
    /**
     * @brief BFS based, get N distributed nodes from the tree.
     * 
     * @param numRequiredPoints 
     * @param pointQueue 
     */
    void ReturnDistributedNPoints(
        const int numRequiredPoints,
        std::vector<T>& pointQueue
    ){
        if (numNodes < numRequiredPoints)
            throw std::runtime_error(std::string("Can not extract enough points."));
        std::deque<std::shared_ptr<Node>> nodeQueue;
        nodeQueue.push_back(mRoot);
        while (nodeQueue.size() < numRequiredPoints){
            std::shared_ptr<Node> firstNode = nodeQueue.front();
            nodeQueue.pop_front();
            if(isLeaf(firstNode.get())){
                nodeQueue.push_back(firstNode);
            }
            else{
                nodeQueue.push_back(firstNode->children[0]);
                if (nodeQueue.size() < numRequiredPoints)
                    break;
                nodeQueue.push_back(firstNode->children[1]);
                if (nodeQueue.size() < numRequiredPoints)
                    break;
                nodeQueue.push_back(firstNode->children[2]);
                if (nodeQueue.size() < numRequiredPoints)
                    break;
                nodeQueue.push_back(firstNode->children[3]);
            }
        }
        for(auto oneNode : nodeQueue){
            pointQueue.push_back(oneNode->values[0]);
        }
        return;
    }
