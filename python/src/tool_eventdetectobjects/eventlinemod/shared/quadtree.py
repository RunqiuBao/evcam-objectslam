...
class Point:
    ...
    def __init__(self, x, y, score, payload=None):
        self.x, self.y = x, y
        self.score = score
...
class QuadTree:
    ...
    def getAllPoints(self, found_points_x, found_points_y, found_points_score):
        """Find all points in the quadtree."""
        for point in self.points:
            found_points_x.append(point.x)
            found_points_y.append(point.y)
            found_points_score.append(point.score)
        # ... and if this node has children, search them too.
        if self.divided:
            found_points_x, found_points_y, found_points_score = self.nw.getAllPoints(found_points_x, found_points_y, found_points_score)
            found_points_x, found_points_y, found_points_score = self.ne.getAllPoints(found_points_x, found_points_y, found_points_score)
    ...
    def queryLeafNode(self, count=0):
        if self.divided:
            self.nw.queryLeafNode(count+1)
            self.ne.queryLeafNode(count+1)
            self.se.queryLeafNode(count+1)
            self.sw.queryLeafNode(count+1)
        elif (len(self.points)!=0):
            print('Level {} node, points number {}'.format(count, len(self.points)))
        else:
            print('Level {} node, empty node'.format(count))
